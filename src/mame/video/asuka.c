...
UINT32 cadash_state::screen_update_cadash(screen_device &screen, bitmap_ind16 &bitmap, const rectangle &cliprect)
{
	UINT8 layer[3];

	m_tc0100scn_2->tilemap_update();

	layer[0] = m_tc0100scn_2->bottomlayer();
	layer[1] = layer[0] ^ 1;
	layer[2] = 2;
	...
	m_tc0100scn_2->tilemap_draw(screen, bitmap, cliprect, layer[0], TILEMAP_DRAW_OPAQUE, 1);
	m_tc0100scn_2->tilemap_draw(screen, bitmap, cliprect, layer[1], 0, 2);
	m_tc0100scn_2->tilemap_draw(screen, bitmap, cliprect, layer[2], 0, 4);

	/* Sprites are always over both bg layers */
	m_pc090oj_2->draw_sprites(bitmap, cliprect, screen.priority(), 0);
	return 0;
}
